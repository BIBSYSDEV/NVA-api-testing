...
Background:
  * def host = ''
  * def basePath = 'https://' + host + '/person/
...
Examples:
  ...
  | existingPerson            | JSON_MEDIA_TYPE    |
...
Scenario Outline: Requesting non-existing resource returns not found error
  ...
  * contentType = responseHeaders['Content-Type'][0]
  ...
  And match response.detail == 'The requested resource ' + basePath + nonExistingPerson + ' does not exist'
  And match response.instance == <NON_EXISTING_RESOURCE_URL>
  And match response.requestId == '#notnull'

Examples:
  | NON_EXISTING_RESOURCE_URL     | CONTENT_TYPE       |
  | basePath + nonExistingPerson  | JSON_LD_MEDIA_TYPE |
  | basePath + nonExistingPerson  | JSON_MEDIA_TYPE    |
...
Examples:
  | VALID_URL              | CONTENT_TYPE       |
  | searchPath + 'Mattias' | JSON_LD_MEDIA_TYPE |
  | searchPath + 'Mattias' | JSON_MEDIA_TYPE    |
...
Scenario Outline: Unexpected error returns Internal Server Error problem response
  ...
  * contentType = responseHeaders['Content-Type'][0]
  Given url <VALID_URL>
  When method get
  And the person API application experiences an unexpected error
  Then status 500
  And match contentType == PROBLEM_JSON_MEDIA_TYPE
...
Scenario Outline: Query with unacceptable method returns Not acceptable error
  ...
  * contentType = responseHeaders['Content-Type'][0]
  ...
  And match response.instance == existingPerson
  And match response.requestId == '#notnull'
...
Examples:
  ...
  | searchPath + 'Mattias' | 'image/jpeg'              |
  | searchPath + 'Mattias' | 'application/xml'         |
  | searchPath + 'Mattias' | 'application/rdf+xml'     |
...
Scenario Outline: Request with content negotiation returns expected response
  ...
  * contentType = responseHeaders['Content-Type'][0]
  ...
  Then status 200
  And match contentType == <CONTENT_TYPE>
  And match response == personResponse
